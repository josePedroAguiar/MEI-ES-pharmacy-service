...
# snippet-end:[python.example_code.sfn.StateMachine_decl]
    ...
    def get_tasks(self, state_machine_arn, execution_arn):
        """
        Get the tasks for a specific execution of a state machine.

        :param state_machine_arn: The ARN of the state machine.
        :param execution_arn: The ARN of the execution.
        :return: The tasks for the execution.
        """
        try:
            ...
            logger.error(
                "Couldn't get tasks for execution %s. Here's why: %s: %s",
                execution_arn, err.response['Error']['Code'], err.response['Error']['Message'])
            raise
        else:
